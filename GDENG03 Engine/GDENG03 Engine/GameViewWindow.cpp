...
void GameViewWindow::onCreate() {
	...
	mSwapChain->initialize(this->mWindowHandle, width, height, false);
...
void GameViewWindow::onUpdate() {
	...
	UINT width = windowRect.right - windowRect.left;
	UINT height = windowRect.bottom - windowRect.top;
	AGraphicsEngine::getInstance()->getImmediateDeviceContext(false)->setViewportSize(width, height);

	GameObjectManager::getInstance()->render(width, height, mVertexShader, mPixelShader, false);
