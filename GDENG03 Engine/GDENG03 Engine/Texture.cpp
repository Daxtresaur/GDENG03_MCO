...
Texture::Texture(const wchar_t* full_path)
{
	...
	HRESULT res = DirectX::LoadFromDDSFile(file_path.c_str(), DirectX::WIC_FLAGS_NONE, nullptr, image_data);
	...
	if (SUCCEEDED(res))
	{
		...
		AGraphicsEngine::getInstance()->getD3DDevice()->CreateShaderResourceView(m_texture, &desc,&m_shader_res_view);
